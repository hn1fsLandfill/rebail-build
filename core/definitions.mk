...
###########################################################
## Find all of the kotlin files under the named directories.
## Meant to be used like:
##    SRC_FILES := $(call all-java-files-under,src tests)
###########################################################

define all-kotlin-files-under
$(call all-named-files-under,*.kt,$(1))
endef
...
$(call dump-words-to-file,$(PRIVATE_JAVA_SOURCES),$(PRIVATE_CLASS_INTERMEDIATES_DIR)/java-source-list)
$(hide) if [ -d "$(PRIVATE_SOURCE_INTERMEDIATES_DIR)" ]; then \
          find $(PRIVATE_SOURCE_INTERMEDIATES_DIR) -name '*.kt' >> $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list; \
fi
$(hide) tr ' ' '\n' < $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list \
    | sort -u > $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list-uniq
$(hide) if [ -s $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list-uniq ] ; then \
    $(1) -encoding UTF-8 \
    $(strip $(PRIVATE_JAVAC_DEBUG_FLAGS)) \
    $(if $(findstring true,$(PRIVATE_WARNINGS_ENABLE)),$(xlint_unchecked),) \
    $(2) \
    ...
    -extdirs "" -d $(PRIVATE_CLASS_INTERMEDIATES_DIR) \
    $(PRIVATE_JAVACFLAGS) \
    \@$(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list-uniq \
    || ( rm -rf $(PRIVATE_CLASS_INTERMEDIATES_DIR) ; exit 41 ) \
fi
$(if $(PRIVATE_JAVA_LAYERS_FILE), $(hide) build/tools/java-layers.py \
    $(PRIVATE_JAVA_LAYERS_FILE) \@$(PRIVATE_CLASS_INTERMEDIATES_DIR)/java-source-list-uniq,)
cat $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list
$(hide) rm -f $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list
$(hide) rm -f $(PRIVATE_CLASS_INTERMEDIATES_DIR)/kotlin-source-list-uniq
endef

define transform-java-to-classes.jar
@echo "target Kotlin: $(PRIVATE_MODULE) ($(PRIVATE_CLASS_INTERMEDIATES_DIR))"
$(call compile-kotlin,$(TARGET_KOTLINC),$(PRIVATE_BOOTCLASSPATH))
